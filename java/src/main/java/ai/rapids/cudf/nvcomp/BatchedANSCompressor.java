...
public class BatchedANSCompressor {
  ...
  public static long chunk_align(long addr) {
    return (addr + (CHUNK_ALIGN - 1L)) & ~(CHUNK_ALIGN - 1L);
  }
  ...
  private long[] calcOutputBufferSizes(int[] chunksPerInput,
    ...
    for (int i = 0; i < sizes.length; i++) {
      ...
      while (chunkIdx < chunkEndIdx) {
        if (chunkIdx == (chunkEndIdx - 1)) {
          bufferSize += outputChunkSizes[chunkIdx++];
        } else {
          bufferSize += chunk_align(outputChunkSizes[chunkIdx++]);
        }
      }
      sizes[i] = bufferSize;
    }
